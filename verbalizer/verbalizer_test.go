...
var _ = Describe("Verbalizer", func() {
	...
	It("verbalizes a simple tweet with URLs and hashtags", func() {
		...
		Expect(t.Show(-1)).To(Equal(
			`Von Peter Goetz: <lang xml:lang="en-US">Just submitted my <break strength="medium"/>hashtag<break strength="medium"/>golang<break strength="medium"/> 2 error handling proposal ` +
				`medium.com, title: "Thinking About New Ways of Error Handling in Go 2 - Peter Goetz - Medium", to the official feedback wiki: ` +
				`github.com, title: "Go2ErrorHandlingFeedback   golang/go Wiki   GitHub",.</lang>`))
	})
	...
	It("verbalizes a simple tweet with URLs and hashtags", func() {
		...
		Expect(t.Show(-1)).To(Equal(
			`Von Danny Trinh: <lang xml:lang="en-US">Tuesday energy: from Gentry Underwood on Navigator. As you add more people to a team, be mindful of all of the new relationships to foster: </lang>`))
	})
	...
	It("verbalizes a retweet", func() {
		...
		Expect(t.Show(-1)).To(Equal(
			`Von AF SMC, retweeted von Elon Musk: ` +
				...
				`</lang><break strength="strong"/>Angehängt zum Tweet ist ein Bild.`))
	})
})
